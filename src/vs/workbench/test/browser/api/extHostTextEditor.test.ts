...
suite('ExtHostTextEditor', () => {
	...
	setup(() => {
		editor = new ExtHostTextEditor('fake', null!, new NullLogService(), new Lazy(() => doc.document), [], { cursorStyle: 0, insertSpaces: true, lineNumbers: 1, tabSize: 4, skipSaveParticipantIds: [] }, [], 1);
	});
	...
	test('API [bug]: registerTextEditorCommand clears redo stack even if no edits are made #55163', async function () {
		...
		let editor = new ExtHostTextEditor('edt1',
			...
			}, new NullLogService(), new Lazy(() => doc.document), [], { cursorStyle: 0, insertSpaces: true, lineNumbers: 1, tabSize: 4, skipSaveParticipantIds: [] }, [], 1);
...
suite('ExtHostTextEditorOptions', () => {
	...
	function assertState(opts: ExtHostTextEditorOptions, expected: IResolvedTextEditorConfiguration): void {
		let actual = {
			...
			skipSaveParticipantIds: opts.value.skipSaveParticipantIds
		};
	...
	test('can set skipSaveParticipantIds to same value', () => {
		opts.value.skipSaveParticipantIds = [];
		assertState(opts, {
			tabSize: 4,
			insertSpaces: false,
			cursorStyle: TextEditorCursorStyle.Line,
	...
	test('can change skipSaveParticipantIds', () => {
		opts.value.skipSaveParticipantIds = ['test'];
		assertState(opts, {
			tabSize: 4,
			insertSpaces: false,
			cursorStyle: TextEditorCursorStyle.Line,
			lineNumbers: RenderLineNumbersType.On,
			skipSaveParticipantIds: ['test']
		});
		assert.deepStrictEqual(calls, [{ skipSaveParticipantIds: ['test'] }]);
	});
